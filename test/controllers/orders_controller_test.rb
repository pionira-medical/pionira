...
class OrdersControllerTest < ActionController::TestCase
  ...
  test "should display the order" do
    ...
    get :sign_in, {id: orders(:one).id}, {order_id: orders(:one).id, order_security_key: orders(:one).security_key}
    assert_redirected_to(controller: :orders, action: :show, id: orders(:one).id)
    #assert content
  end
  
  test "should redirect to sign in page if the params id is not valid" do
    get :show, {id: '0000'}, {order_id: orders(:one).id, order_security_key: orders(:one).security_key}
    assert_redirected_to(controller: :orders, action: :sign_in, id: '0000')
